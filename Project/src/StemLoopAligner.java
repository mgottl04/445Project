...
public class StemLoopAligner {
	...
	public static double alignStemLoops(StemLoop a, StemLoop b)
		...
		for (int i = 1; i < indexingPairsA.size(); i++) {
			...
			// Determine type
			StemLoop.NodeType typex = a.getType(x);
			StemLoop.NodeType typey = a.getType(y);
			boolean bothAInternal = (typex == StemLoop.NodeType.INTERNAL)
					&& (typey == StemLoop.NodeType.INTERNAL);
			...
			for (int j = 1; j < indexingPairsB.size(); j++) {
				...
				// Determine type
				StemLoop.NodeType typeu = b.getType(u);
				StemLoop.NodeType typev = b.getType(v);
				boolean bothBInternal = (typeu == StemLoop.NodeType.INTERNAL)
						&& (typev == StemLoop.NodeType.INTERNAL);

				// Compute D[i][j] depending on type
				
				if (bothAInternal && bothBInternal) {
					double valA = D[px_sy_index][pu_sv_index] + EditCosts.relableCost(x,u);
					double valB = D[px_sy_index][u_v_index] + EditCosts.deleteCost(x);
					double valC = D[x_y_index][pu_sv_index] + EditCosts.deleteCost(u);
					...
					valA = valA < 0 ? valA = Math.log(0) : valA + EditCosts.deleteCost(x);
					double valB = D[x_y_index][pu_v_index];
					valB = valB < 0 ? valB = Math.log(0) : valB + EditCosts.insertCost(u);
					double valC = D[x_y_index][u_sv_index];
					valC = valC < 0 ? valC = Math.log(0) : valC + EditCosts.insertCost(v);
					double valD = D[px_sy_index][pu_v_index];
					valD = valD < 0 ? valD = Math.log(0) : valD + EditCosts.alteringCost(x,u);
					double valE = D[px_sy_index][u_sv_index];
					valE = valE < 0 ? valE = Math.log(0) : valE + EditCosts.alteringCost(x,v);
					double valF = D[px_sy_index][pu_sv_index];
					valF = valF < 0 ? valF = Math.log(0) : valF + EditCosts.arcBreakingCost(x,u,v);
					...
					valA = valA < 0 ? valA = Math.log(0) : valA + EditCosts.insertCost(u);
					double valB = D[px_y_index][u_v_index];
					valB = valB < 0 ? valB = Math.log(0) : valB + EditCosts.deleteCost(x);
					double valC = D[x_sy_index][u_v_index];
					valC = valC < 0 ? valC = Math.log(0) : valC + EditCosts.deleteCost(y);
					double valD = D[px_y_index][pu_sv_index];
					valD = valD < 0 ? valD = Math.log(0) : valD + EditCosts.completionCost(x,u);
					double valE = D[x_sy_index][pu_sv_index];
					valE = valE < 0 ? valE = Math.log(0) : valE + EditCosts.completionCost(y,u);
					double valF = D[px_sy_index][pu_sv_index];
					valF = valF < 0 ? valF = Math.log(0) : valF + EditCosts.arcCreationCost(x,y,u);
					...
					valB = valB < 0 ? valB = Math.log(0) : valB + EditCosts.deleteCost(y);
					double valC = D[x_y_index][pu_v_index];
					valC = valC < 0 ? valC = Math.log(0) : valC + EditCosts.insertCost(u);
					double valD = D[x_y_index][u_sv_index];
					valD = valD < 0 ? valD = Math.log(0) : valD + EditCosts.insertCost(v);
					double valE = D[px_y_index][u_v_index];
					valE = valE < 0 ? valE = Math.log(0) : valE + EditCosts.relableCost(x,u);
					double valF = D[x_sy_index][u_sv_index];
					valF = valF < 0 ? valF = Math.log(0) : valF + EditCosts.relableCost(y,v);
